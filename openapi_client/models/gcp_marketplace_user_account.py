...
class GcpMarketplaceUserAccount(BaseModel):
    ...
    """
    approvals: Optional[conlist(GcpMarketplaceUserAccountApproval)] = Field(None, description="The approvals for this account, that are permitted or have been completed.")
    billing_account_id: Optional[StrictStr] = Field(None, alias="billingAccountId", description="The buyer's GCP billing account ID.")
    company_info: Optional[CompanyInfo] = Field(None, alias="companyInfo")
    create_time: Optional[datetime] = Field(None, alias="createTime", description="RFC3339 UTC timestamp")
    id: Optional[StrictStr] = Field(None, description="GCP Marketplace User Account ID.")
    input_properties: Optional[conlist(StrictInt)] = Field(None, alias="inputProperties")
    name: Optional[StrictStr] = Field(None, description="The resource name of the account. Account names have the form providers/{provider_id}/accounts/{account_id}.")
    provider: Optional[StrictStr] = Field(None, description="The identifier of the service provider (SaaS Seller) that this account was created against.")
    state: Optional[GcpMarketplaceUserAccountState] = None
    update_time: Optional[datetime] = Field(None, alias="updateTime", description="RFC3339 UTC timestamp")
    user_info: Optional[GcpUserInfo] = Field(None, alias="userInfo")
    __properties = ["approvals", "billingAccountId", "companyInfo", "createTime", "id", "inputProperties", "name", "provider", "state", "updateTime", "userInfo"]
    ...
    def from_dict(cls, obj: dict) -> GcpMarketplaceUserAccount:
        ...
        _obj = GcpMarketplaceUserAccount.parse_obj({
            "approvals": [GcpMarketplaceUserAccountApproval.from_dict(_item) for _item in obj.get("approvals")] if obj.get("approvals") is not None else None,
            ...
            "user_info": GcpUserInfo.from_dict(obj.get("userInfo")) if obj.get("userInfo") is not None else None
        })
        return _obj
