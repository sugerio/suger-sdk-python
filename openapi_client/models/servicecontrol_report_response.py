...
class ServicecontrolReportResponse(BaseModel):
    ...
    """
    report_errors: Optional[conlist(ServicecontrolReportError)] = Field(None, alias="reportErrors", description="ReportErrors: Partial failures, one for each `Operation` in the request that failed processing. There are three possible combinations of the RPC status: 1. The combination of a successful RPC status and an empty `report_errors` list indicates a complete success where all `Operations` in the request are processed successfully. 2. The combination of a successful RPC status and a non-empty `report_errors` list indicates a partial success where some `Operations` in the request succeeded. Each `Operation` that failed processing has a corresponding item in this list. 3. A failed RPC status indicates a general non-deterministic failure. When this happens, it's impossible to know which of the 'Operations' in the request succeeded or failed.")
    service_config_id: Optional[StrictStr] = Field(None, alias="serviceConfigId", description="ServiceConfigId: The actual config id used to process the request.")
    service_rollout_id: Optional[StrictStr] = Field(None, alias="serviceRolloutId", description="ServiceRolloutId: The current service rollout id used to process the request.")
    ...
    def from_dict(cls, obj: dict) -> ServicecontrolReportResponse:
        ...
        _obj = ServicecontrolReportResponse.parse_obj({
            "report_errors": [ServicecontrolReportError.from_dict(_item) for _item in obj.get("reportErrors")] if obj.get("reportErrors") is not None else None,
