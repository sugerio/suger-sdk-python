...
class SlackIntegration(BaseModel):
    ...
    authed_user: Optional[SlackOAuthV2ResponseAuthedUser] = Field(
        None, alias="authedUser"
    )
    ...
    incoming_webhook: Optional[SlackOAuthResponseIncomingWebhook] = Field(
        None, alias="incomingWebhook"
    )
        ...
        _obj = SlackIntegration.parse_obj(
            {
                ...
                "authed_user": SlackOAuthV2ResponseAuthedUser.from_dict(
                    obj.get("authedUser")
                )
                if obj.get("authedUser") is not None
                else None,
                "bot_user_id": obj.get("botUserId"),
                "enterprise": SlackOAuthV2ResponseEnterprise.from_dict(
                    obj.get("enterprise")
                )
                if obj.get("enterprise") is not None
                else None,
                "expires_in": obj.get("expiresIn"),
                "incoming_webhook": SlackOAuthResponseIncomingWebhook.from_dict(
                    obj.get("incomingWebhook")
                )
                if obj.get("incomingWebhook") is not None
                else None,
                "redirect_url": obj.get("redirectUrl"),
                "refresh_token": obj.get("refreshToken"),
                "scope": obj.get("scope"),
                "team": SlackOAuthV2ResponseTeam.from_dict(obj.get("team"))
